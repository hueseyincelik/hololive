...
class GUI:
	def __init__(self, ip, port, remote=True, camera='CCD', exposure_time=0.2, binning=4, dimension=512, real_FFT=False):
		...
		while True:
			for event in pg.event.get():
				if event.type == pg.QUIT:
					...
					if event.key == pg.K_s:
						self.save_screenshot_thread = threading.Thread(target=self.save_screenshot)
			...
			if not self.pause:
				...
				if self.unwrap_phase and not self.reconstruct_amplitude:
					self.current_reconstruction = restoration.unwrap_phase(self.current_reconstruction)
			...
			self.font.render_to(self.screen, (5, 5), f"Filter: {self.butterworth_filter} ({np.round(self.butterworth_cutoff, 2)})", pg.Color('RED'))
			self.font.render_to(self.screen, (5, 25), f"Phase: {self.phase_amplification}x{' (U)' if self.unwrap_phase else ''}", pg.Color('RED'))
			self.font.render_to(self.screen, (5, 45), f"Mask: {self.centerband_mask}%", pg.Color('RED'))
			self.font.render_to(self.screen, (5, 85), f"Sideband: {self.sideband_area}{' (L)' if self.sideband_lock else ''}", pg.Color('RED'))
			self.font.render_to(self.screen, (5, 105), f"Contrast: {np.round(100 * self.fringe_contrast, 2)}%", pg.Color('RED'))
	...
	def reconstruct(self):
		...
		img_fft = image.rfft2_to_fft2(self.img_CCD.shape, sfft.rfft2(self.img_CCD)) if self.real_FFT else sfft.fft2(self.img_CCD)
		...
		cb_rr, cb_cc = draw.disk(np.asarray(img_fft_shifted.shape) / 2, min(img_fft_shifted.shape) * (self.centerband_mask / 100))
		img_fft_shifted[cb_rr, cb_cc] = 0

		if not self.sideband_lock:
			self.sideband_position = np.unravel_index(img_fft_shifted.argmax(), img_fft_shifted.shape)
			self.sideband_distance = np.linalg.norm(np.asarray(img_fft_shifted.shape) / 2 - self.sideband_position)

		self.fringe_contrast = 2 * np.abs(img_fft_shifted[tuple(self.sideband_position)]) / np.abs(img_fft[0, 0])
	...
	def save_screenshot(self, extension='png'):
		pg.image.save(self.screen, f"HoloLive_{'PH' if not self.reconstruct_amplitude else 'AMP'}_{format(datetime.datetime.now(), '%Y-%m-%d_%H-%M-%S')}.{extension}")
